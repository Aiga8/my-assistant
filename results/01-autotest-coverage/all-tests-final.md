...
**Ожидаемый результат:**
- Адрес страницы не указывает на открытие конкретного дашборда (не происходит переход на `/dashboards/<dashboardUuid>` для дашборда, где Table-виджет не в режиме SlideSheet)
...
**Ожидаемый результат:**
- При некорректном идентификаторе дашборда отображается страница 404/ошибки (`[data-testid="dashboard-404-page"]` видим)
- При неполном адресе `/projects/<projectUuid>/dashboards/` выполняется переход на корректный дашборд из сохранённых настроек:
  - адрес страницы содержит формат `/dashboards/` или `/windows/`
  - адрес страницы содержит идентификатор дашборда по умолчанию
...
**TestRail:**
- ID: 61472 (partial), уже выставлен как Automated
- Теги: _(заполнить позже)_
...
**TestRail:**
- ID: 61472 (partial), Automated был уже установлен
- Теги: _(заполнить позже)_
...
**TestRail:**
- ID: 77802, Automated был уже установлен
- Теги: _(заполнить позже)_
