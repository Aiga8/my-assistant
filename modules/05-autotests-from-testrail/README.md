...
### Шаг 5 — Проверка DOM и `data-testid`/`test-id` (при необходимости)

Если DOM/HTML фрагменты не предоставлены и по описанию неясно, есть ли у нужных элементов `data-testid`/`test-id`, сначала выдай:
...
Ты ОБЯЗАН соблюдать внутреннее правило из DS-9203: для автотестов локаторы должны быть на `data-testid`/`test-id`.

Правила:
- DOM-first: перед тем как писать тест, убедись, что у всех нужных элементов есть `data-testid`/`test-id`.
...
- Если нужного `data-testid`/`test-id` нет в приложении — это MUST:
