...
`modules/10-autotest-coverage/testrail_export_section_subtree.mjs`
...
| `--out` | `-o` | нет | Папка для результата. По умолчанию: `results/10-autotest-coverage`. |
...
```bash
node modules/10-autotest-coverage/testrail_export_section_subtree.mjs --project 5 --root-section <SECTION_ID> --out results/10-autotest-coverage
```

С указанием своей папки:

```bash
node modules/10-autotest-coverage/testrail_export_section_subtree.mjs -p 5 -s 22177 -o results/10-autotest-coverage
```
...
- **Путь:** `results/10-autotest-coverage/` (или значение `--out`).
- **Имена:**
...
Задача: Этап 2 маппинга автотестов на TestRail. Выгрузи кейсы TestRail для указанной секции и всех дочерних. Project_id = 5. Запусти скрипт modules/10-autotest-coverage/testrail_export_section_subtree.mjs с --project 5 и --root-section <SECTION_ID>, результат сохрани в results/10-autotest-coverage/. Дальше по инструкции в modules/10-autotest-coverage/02_export_testrail_section_subtree.md.
```
...
2. Запусти экспорт:
   ...
   node modules/10-autotest-coverage/testrail_export_section_subtree.mjs --project 5 --root-section <SECTION_ID> --out results/10-autotest-coverage
   ```
3. Результат: в `results/10-autotest-coverage/` появятся `testrail-section-<SECTION_ID>-subtree-cases.json` и `.md` — они нужны для Этапа 3.
