...
- **Описания автотестов:** Markdown-файл из Этапа 1  
  `results/10-autotest-coverage/<номер_задачи>_<область>.md`  
  либо сабсет: `results/10-autotest-coverage/all-tests-<subset>-only.md`.
- **Выгрузка кейсов TestRail:** JSON из Этапа 2  
  `results/10-autotest-coverage/testrail-section-<SECTION_ID>-subtree-cases.json`
...
Сабсет сохраняется, например, в `results/10-autotest-coverage/all-tests-<subset>-only.md` и передаётся в скрипт маппинга как `--input`.
...
`modules/10-autotest-coverage/map_autotests_to_testrail.mjs`
...
```bash
node modules/10-autotest-coverage/map_autotests_to_testrail.mjs --input results/10-autotest-coverage/QAA-6265_trajectory.md --cases results/10-autotest-coverage/testrail-section-6277-subtree-cases.json --out-base results/10-autotest-coverage/trajectory-section6277-testrail-mapping-12_02_25
```

С указанием своей папки/префикса:

```bash
node modules/10-autotest-coverage/map_autotests_to_testrail.mjs ^
  --input results/10-autotest-coverage/all-tests-<subset>-only.md ^
  --cases results/10-autotest-coverage/testrail-section-<SECTION_ID>-subtree-cases.json ^
  --out-base results/10-autotest-coverage/<subset>-section<SECTION_ID>-testrail-mapping-<dd_mm_yy>
```
...
- По каждому запуску маппинга (сабсету):  
  `<out-base>.md`, `<out-base>.json` в `results/10-autotest-coverage/`.
- Агрегированный рабочий файл (один на все тесты):  
  `results/10-autotest-coverage/all-tests-testrail-mapping.md`
...
- Входной файл описаний: `results/10-autotest-coverage/<файл>.md` или сабсет.
- Рабочий файл результата: `results/10-autotest-coverage/all-tests-testrail-mapping.md`.
...
2. При необходимости подготовить сабсет тестов под эту секцию (по path/ключевым словам), сохранить в `results/10-autotest-coverage/`.
3. Запустить маппинг: `modules/10-autotest-coverage/map_autotests_to_testrail.mjs` с `--input`, `--cases`, `--out-base` в `results/10-autotest-coverage/`.
